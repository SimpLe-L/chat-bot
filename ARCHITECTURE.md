...
- GitHub/Google：`GET /api/auth/oauth/{provider}` 返回授权 URL 并设置 10 分钟有效的 HttpOnly OAuth state cookie；`/redirect` 可直接跳转；`/callback` 校验 state 后交换 token、创建用户并签发 session cookie。生产部署需要配置对应 client id/secret 和回调域名。
...
- GitHub/Google OAuth 已打通 state-cookie 授权流；生产部署前需要填 client id/secret，并确认回调域名与 OAuth App 配置一致。
